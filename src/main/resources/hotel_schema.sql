...
CREATE TABLE IF NOT EXISTS USER_ROLE (
  role_id INTEGER AUTO_INCREMENT UNIQUE,
  type    VARCHAR(10) NOT NULL,
  PRIMARY KEY (role_id)
);

CREATE TABLE IF NOT EXISTS GUEST (
  identity_serial VARCHAR(18) NOT NULL UNIQUE,
  name            VARCHAR(10),
...
CREATE TABLE IF NOT EXISTS REGISTERED_USER (
  phone_number  VARCHAR(11) NOT NULL UNIQUE COMMENT '用户手机号，可用于登录',
...
CREATE TABLE IF NOT EXISTS BOOKING_TRANSITION (
  ...
  FOREIGN KEY (user_phone_number) REFERENCES REGISTERED_USER (phone_number)
    ON DELETE CASCADE,
